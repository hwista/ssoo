...
function checkLocation(filePath) {
  ...
  if (parts.length === 2 &&
      ['common', 'pms', 'dms', 'sns', 'crm'].includes(parts[0]) &&
  ...
  const domain = parts[0]; // common, pms, dms, sns, crm
  const directory = parts[1]; // tutorials, guides, explanation, etc.

  // 도메인 확인
  if (!['common', 'pms', 'dms', 'sns', 'crm'].includes(domain)) {
    errors.push({
      type: 'location',
      message: `알 수 없는 도메인: ${domain} (common, pms, dms, sns, crm만 허용)`,
      severity: 'error',
    });
    return errors;
  }

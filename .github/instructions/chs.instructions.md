...
> 이 규칙은 `apps/web/chs/` 경로의 파일 작업 시 적용됩니다.
...
| TabBar | ❌ 없음 | CHS는 페이지 라우팅 사용 |
| Control 높이 | 36px (`h-control-h`) | PMS와 동일 |

### PMS와의 레이아웃 차이

CHS는 MDI 탭 시스템 대신 **페이지 라우팅**을 사용합니다:
...
- raw 타이포 조합(`text-sm font-medium`, `text-xs font-medium`, `text-sm font-semibold`, `text-lg font-semibold`, `text-xs font-semibold`, `text-lg font-medium`, `body-text font-medium`)과 개별 `fontFamily` 하드코딩은 금지합니다.
...
export const postsApi = {
  feed: (params?: FeedQueryDto) =>
    client.get<PaginatedResponse<FeedItem>>('/chs/feed', { params }),
...
export function useCreatePost() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['chs', 'feed'] });
    },
  });
}
```
...
- `docs/chs/README.md` — CHS 문서 허브
- `docs/chs/CHANGELOG.md` — 변경 이력

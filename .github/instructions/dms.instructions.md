...
- `codex:dms-publish`는 GitHub 브랜치 push 전에 GitLab subtree fast-forward 가능 여부를 먼저 검사한 뒤, GitHub 브랜치 push + GitLab subtree push + 해시 검증까지 수행합니다.
- DMS 변경 상태에서 `origin` push 시 `codex:dms-publish` 마커가 없으면 pre-push가 차단됩니다.
- GitLab 브랜치가 로컬 subtree보다 앞서 있으면 먼저 `pnpm run codex:dms-sync-from-gitlab`로 monorepo에 재통합합니다.
...
- 수행 내용: GitLab subtree 선검사 → GitHub 브랜치 push → GitLab subtree push → 해시 검증
- GitLab subtree가 앞서 있으면 `pnpm run codex:dms-sync-from-gitlab`로 먼저 재통합

...
| **DMS runtime content paths** | `apps/web/dms/dms.config.default.json`, `compose.yaml`, `DMS_*` env | markdown working tree, template root, ingest queue, binary storage |
...
- 문서 데이터 경로 확인 시 → `apps/web/dms/dms.config.default.json`, `compose.yaml`, `DMS_*` runtime binding 참조

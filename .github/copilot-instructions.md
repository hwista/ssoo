...
- 사용자 지시 범위만 수행하고, 임의로 작업 범위를 확장하지 않습니다.
- 요청이 애매하거나 해석이 갈리는 경우, 추정 진행 대신 먼저 확인 질문을 합니다.
- 과대 해석으로 사용자 의도와 다른 작업을 진행하는 행위를 금지합니다.
...
### 10. 비판적 수용 (무조건적 긍정 금지)

- **사용자 요청이라도 기술적 타당성을 반드시 검증** 후 수행
- "좋은 접근입니다" 등 무조건적 긍정 금지 → 문제점이 있으면 먼저 지적
- 요청이 기존 아키텍처, 패턴, 규칙과 충돌하면 **충돌 사실을 먼저 보고**
- 대안이 더 나은 경우 **대안을 제시하고 사용자 선택**을 받음
- 비판 없는 수용은 품질 저하의 원인 → 에이전트는 기술 리뷰어 역할 수행

### 11. 기존 결과 보존 (회귀 방지)

- **새 작업이 기존 기능·동작·UI 외형을 왜곡·축소·변형해서는 안 됨**
- 코드 수정이 아닌 기능적/시각적 결과물의 무결성 보존이 핵심
- 기존 동작에 영향을 줄 수 있는 변경 시 **영향 범위를 먼저 분석하고 보고**
- 의도치 않은 회귀가 발생하면 즉시 원복하고 다른 접근법 모색
- 변경 전/후 동작 차이가 있으면 사용자에게 **명시적으로 고지**

### 12. 워크스페이스 패턴 최우선 + 경계 관리

- **워크스페이스에 정의된 코드 스타일·디자인 룰·패턴을 동일하게 따르는 것이 최우선**
- 새 코드는 기존 코드와 **동일한 패턴, 동일한 구조**로 작성
- 각 모듈/컴포넌트의 **역할과 책임에 명확한 경계**를 유지
- 하나의 파일/컴포넌트/모듈이 **비대해지지 않도록 관리** (컴포넌트 크기 가이드 준수)
- 경계가 모호하거나 덩치가 커지면 **분리를 먼저 제안**
...
> 이 레포에서는 3개의 AI 코딩 에이전트가 동시에 사용됩니다.

### 에이전트별 Instruction 위치

| 에이전트 | 정본/미러 | 위치 |
|---------|----------|------|
| **GitHub Copilot** | 정본 | `.github/copilot-instructions.md`, `.github/instructions/` |
| **Codex CLI** | 미러 | `.codex/instructions/` |
| **Claude Code** | 미러 | `CLAUDE.md` (루트), `apps/web/dms/CLAUDE.md` (DMS) |

### 규칙 변경 프로세스

1. **정본 먼저**: `.github/` 문서 수정
2. **Codex 미러 반영**: `.codex/instructions/` 대응 파일 업데이트
3. **Claude 미러 반영**: `CLAUDE.md` 대응 섹션 업데이트
4. **검증 실행**: `node .codex/scripts/verify-codex-sync.js`
5. **검증 실패 시 커밋 차단**

### 동기화 검증

- 매니페스트: `.codex/config/sync-manifest.json`
- 스크립트: `node .codex/scripts/verify-codex-sync.js`
- 핵심 섹션 (원칙, 금지사항, 네이밍, 레이어) 의미 동일 유지 필수

...
SSOO(삼삼오오)는 SI/SM 조직의 **Opportunity -> Project -> System** 흐름을 하나로 묶는 pnpm workspace + Turborepo 모노레포입니다. 서버와 PMS/CMS/DMS/Admin 웹 앱이 공유 타입, 공용 인증 런타임, 공용 웹 셸을 함께 사용하고, 프로세스 정본은 `.github/`, 산출물 문서는 `docs/`와 `docs/dms/`에 둡니다.
...
| Docker 전체 스택 | `pnpm docker:up` | postgres + db-init + server + PMS + DMS + CMS + Admin |
...
- DB는 Prisma multi-schema(`common`, `pms`, `dms`, `cms`)를 사용합니다. 스키마 간 직접 FK 대신 애플리케이션 레벨 조인을 유지하고, 새 영속 모델은 필요 시 원본 테이블과 `_h` 히스토리 테이블/트리거를 함께 다룹니다.
...
- PMS/CMS/DMS/Admin의 `src/stores/auth.store.ts`는 모두 `createAuthStore(...)`를 감싼 thin wrapper입니다.
- 각 앱의 `src/app/(main)/layout.tsx`는 `useProtectedAppBootstrap(...)`로 hydration, auth 확인, access hydrate, redirect를 공통 처리합니다.
- PMS/DMS는 기본 `AuthIdentity`를 그대로 쓰고, CMS/Admin은 profile display 필드를 덧붙인 `AuthUser`를 씁니다.

...
- shared rule은 `scripts/copilot-issue/**`, 이 가이드, issue template, shared prompt wrapper에 둔다. 개인 agent 파일이나 machine-local overlay에는 공통 규칙을 저장하지 않는다.
...
- stateful 로직은 항상 `scripts/copilot-issue/**`와 `codex:workspace-*` publish script를 호출한다.
...
- duplicate semantic 판단은 deterministic title/body heuristic이 아니라 interactive AI/operator 판단으로 확정한다.
- duplicate suspected issue는 triage 단계에서 자동 close하지 않는다.
- merge transport 자체는 새로 만들지 않고 `WS-011` safe merge/publish 경로를 재사용한다.

  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <Card>
        ...
        <CardContent className="relative pt-0 pb-4">
          <Avatar className="h-24 w-24 -mt-12 border-4 border-white shadow-lg">
            ...
            <AvatarFallback className="bg-ssoo-primary text-white text-title-section">
              {initials}
            </AvatarFallback>
          </Avatar>
          <div className="mt-3">
            <h1 className="text-title-subsection">{user?.displayName || user?.userName}</h1>
            {user?.positionCode && (
            ...
            {profile.bio && <p className="mt-2 text-body-sm">{profile.bio}</p>}
      ...
      {profile.skills && profile.skills.length > 0 && (
        <Card>
          <CardHeader><CardTitle>스킬</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {profile.skills.map((skill) => (
              <div key={skill.id} className="flex items-center justify-between">
                <div>
                  <span className="text-label-md">{skill.skillName}</span>
                  <Badge variant="outline" className="ml-2">{skill.skillCategory}</Badge>
                </div>
      ...
      {profile.careers && profile.careers.length > 0 && (
        <Card>
          <CardHeader><CardTitle>프로젝트 이력</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {profile.careers.map((career, i) => (
              <div key={career.id}>
                {i > 0 && <Separator className="mb-4" />}
                <div>
                  <h4 className="text-label-strong">{career.projectName}</h4>
                  <p className="text-caption text-muted-foreground">{career.roleName}</p>
                  {career.companyName && (
                    <p className="text-caption text-muted-foreground">{career.companyName}</p>
                  )}

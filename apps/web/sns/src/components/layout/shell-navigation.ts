...
export function getSnsShellSection(pathname: string): SnsShellSectionKey {
  const userSurfaceRoute = parseSsooUserSurfaceRouteEntry(pathname);
  if (userSurfaceRoute) {
    return userSurfaceRoute.kind === 'personal-settings' ? 'settings' : 'profile';
  }
...
export function getSnsShellTabOptions(path: string) {
  ...
  const normalizedPath = normalizeSsooUserSurfaceRouteEntryPath(rawPath) ?? rawPath;
...
export function getSnsShellTabIcon(path: string) {
  const userSurfaceRoute = parseSsooUserSurfaceRouteEntry(path);
  if (userSurfaceRoute) {
    return userSurfaceRoute.kind === 'personal-settings' ? Settings : UserRound;
  }

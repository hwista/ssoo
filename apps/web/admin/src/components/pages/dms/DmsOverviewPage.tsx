          ...
          {isLoading ? (
            ...
            <Table className="w-full text-sm">
              <TableHeader className="border-b bg-muted/40">
                <TableRow>
                  <TableHead className="px-4 py-2 text-left font-medium">사용자</TableHead>
                  <TableHead className="px-4 py-2 text-right font-medium">문서 수</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {overview.topOwners.map((row) => (
                  <TableRow key={row.ownerUserId} className="border-b last:border-b-0">
                    <TableCell className="px-4 py-2">
                      {row.loginId ?? <span className="text-muted-foreground">user#{row.ownerUserId}</span>}
                    </TableCell>
                    <TableCell className="px-4 py-2 text-right font-semibold">{row.documentCount}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>

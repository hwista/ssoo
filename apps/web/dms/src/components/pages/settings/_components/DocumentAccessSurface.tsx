      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="grid min-w-[180px] gap-2 text-right text-caption text-ssoo-primary/70 sm:grid-cols-2 sm:text-left">
          <div className="rounded-md border border-ssoo-content-border bg-ssoo-content-bg/30 px-3 py-2">
            <p className="text-badge text-ssoo-primary/60">대기 요청</p>
            <p className="mt-1 text-label-strong text-ssoo-primary">{document.requestSummary.pending}</p>
          </div>
          <div className="rounded-md border border-ssoo-content-border bg-ssoo-content-bg/30 px-3 py-2">
            <p className="text-badge text-ssoo-primary/60">유효 권한</p>
      ...
      {document.syncStatusCode === 'repair_needed' && document.repairReason && (
        <div className="mt-3 rounded-lg border px-4 py-3 text-body-sm ssoo-tone-warning-surface">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-medium">이 문서는 문서 목록 연동 정보 보정이 필요합니다.</p>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {document.grants.length > 0 && (
          ...
          >
            ...
            권한 상세 ({document.grants.length})
          </Button>
        )}
      </div>

      {showTransferForm && (
        <div className="mt-3 rounded-lg border border-ssoo-content-border bg-ssoo-content-bg/30 px-4 py-3 [container-type:inline-size]">
          <p className="text-caption text-ssoo-primary/70">
            새 소유자의 로그인 아이디를 입력하세요. 현재 소유자의 기존 권한은 유지됩니다.
          </p>
          <div className="mt-2 flex items-center gap-2 [@container(max-width:399px)]:flex-wrap">
            <Input
              type="text"
              value={transferLoginId}
              onChange={(event) => setTransferLoginId(event.target.value)}
              placeholder="새 소유자 로그인 아이디"
              className="min-w-0 flex-1 [@container(max-width:399px)]:basis-full border-ssoo-content-border bg-card text-ssoo-primary focus:border-ssoo-primary/40 focus-visible:ring-ssoo-primary/10"
            />
            <Button
              size="sm"
      ...
      {request.canRespond && request.status === 'pending' && (
        <div className="mt-4 space-y-3 rounded-lg border border-ssoo-content-border bg-ssoo-content-bg/30 px-4 py-3">
          <div className="grid gap-3 lg:grid-cols-[1fr_180px]">
            ...
            <label className="space-y-1">
              <span className="text-caption text-ssoo-primary/70">권한 만료일 (선택)</span>
              <Input
                type="date"
  ...
  return (
    <section id={anchorId} className="scroll-mt-4 space-y-3 rounded-lg border border-ssoo-content-border bg-card px-4 py-3">
      <header>
        ...
        <p className="text-caption text-ssoo-primary/70">
          별도 요청 없이 특정 사용자에게 문서 읽기 또는 쓰기 권한을 즉시 부여합니다. 사용자 식별자는 관리자 화면 등에서 확인하세요.
        </p>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <label className="block">
          <span className="text-caption text-ssoo-primary/70">대상 사용자 식별자</span>
          <Input
            type="text"
...
export function DocumentAccessSurface({ anchorIds = {} }: { anchorIds?: Record<string, string> }) {
  ...
  const handleRevokeGrant = async (documentId: string, grantId: string) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : '권한 회수에 실패했습니다.');
    }
  };
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {canManageDocumentAccess ? (
          <>
            ...
            <SummaryCard title="승인 요청함" value={counts.inbox} icon={<Inbox className="h-5 w-5" />} />
          </>
        ) : null}
        {canViewMyRequests ? (
          <>
      ...
      {canManageDocumentAccess ? (
        <>
          <section id={anchorIds['manageable-documents']} className="scroll-mt-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-caption text-ssoo-primary/70">
                  소유자 또는 관리 권한 기준으로 내가 권한을 운영할 수 있는 문서 목록입니다.
                </p>
              </div>
          ...
          <section id={anchorIds['approval-inbox']} className="scroll-mt-4 space-y-3">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h3 className="text-label-strong text-ssoo-primary">승인 대기함</h3>
                <p className="text-caption text-ssoo-primary/70">
                  내가 관리 가능한 문서에 들어온 승인 대기 요청만 표시합니다.
                </p>
              </div>

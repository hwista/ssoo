...
function getBackendErrorMessage(responseBody: BackendSuccessResponse<unknown> | BackendErrorResponse | null): string {
  if (!responseBody || responseBody.success === true) {
    return '서버 문서 보조 처리 중 오류가 발생했습니다.';
  }

  return responseBody.error?.message || responseBody.message || '서버 문서 보조 처리 중 오류가 발생했습니다.';
}

export async function POST(req: Request) {
  const body = await req.json();
  const response = await fetch(createServerApiUrl('/dms/doc-assist'), createServerApiProxyInit(req, {
    method: 'POST',
    headers: {
  ...
  if (body?.stream !== false && body?.action !== 'recommendPath' && body?.action !== 'recommendTitleAndPath') {

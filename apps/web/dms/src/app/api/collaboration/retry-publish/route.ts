...
export async function POST(req: Request) {
  ...
  const response = await fetch(createServerApiUrl('/dms/collaboration/retry-publish'), createServerApiProxyInit(req, {
    method: 'POST',
  ...
  if (!response.ok || !responseBody || responseBody.success !== true) {
    return Response.json({ error: getBackendErrorMessage(responseBody, 'publish 재시도 처리 중 오류가 발생했습니다.') }, { status: response.status || 500 });
  }

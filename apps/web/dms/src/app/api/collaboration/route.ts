...
function getBackendErrorMessage(responseBody: BackendSuccessResponse<unknown> | BackendErrorResponse | null): string {
  if (!responseBody || responseBody.success === true) {
    return '협업 상태 처리 중 오류가 발생했습니다.';
  }
  return (typeof responseBody.error === 'string' ? responseBody.error : responseBody.error?.message) || responseBody.message || '협업 상태 처리 중 오류가 발생했습니다.';
}

export async function GET(req: Request) {
  const query = new URL(req.url).searchParams.toString();
  const pathname = query ? `/dms/collaboration?${query}` : '/dms/collaboration';
...
export async function POST(req: Request) {
  ...
  const response = await fetch(createServerApiUrl('/dms/collaboration'), createServerApiProxyInit(req, {
    method: 'POST',

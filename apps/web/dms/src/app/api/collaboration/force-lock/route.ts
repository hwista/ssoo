...
function getBackendErrorMessage(responseBody: BackendSuccessResponse<unknown> | BackendErrorResponse | null): string {
  if (!responseBody || responseBody.success === true) {
    return '협업 강제 잠금 처리 중 오류가 발생했습니다.';
  }
  return (typeof responseBody.error === 'string' ? responseBody.error : responseBody.error?.message) || responseBody.message || '협업 강제 잠금 처리 중 오류가 발생했습니다.';
}

export async function POST(req: Request) {
  const body = await req.json();
  const response = await fetch(createServerApiUrl('/dms/collaboration/force-lock'), createServerApiProxyInit(req, {
    method: 'POST',

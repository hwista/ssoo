...
**저장 시 폴더 변경 수정 즉시 저장되는 거 수정**/폴더명 직접 지정할 수 있게 적용/ 폴더나 문서명 변경돼도 사이드카에 하이라이트 되도록 변경/ **파일명 및 경로 ai 추천**
문서명 위에 공개범위 지정하도록 수정
태그 선택 시 #태그 검색 추가/ 태그 검색 기능 추가
**ai 작성 시 신규냐 편집이냐에 따라 문서명/경로/태그/요약 에 자동 완성 및 자동 추천**
대댓글 기능 추가/ 댓글에 프로필 추가
...
ai 챗 플로팅 버튼 이동가능하게 하고 위치 딱 정해놓고/ 내부에 플로팅 버튼 외부로 빼서 내부 가독성 향상 시키기.

...
import type { DeepPartial, StorageProvider } from '@/server/services/config/ConfigService';
...
class PersonalSettingsService {
  ...
  constructor() {
    this.settingsPath = path.join(process.cwd(), PERSONAL_SETTINGS_FILE);
    this.defaultSettingsPath = path.join(process.cwd(), DEFAULT_PERSONAL_SETTINGS_FILE);
  }

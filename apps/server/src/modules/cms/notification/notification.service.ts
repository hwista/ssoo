...
export class NotificationService {
  ...
  async findAll(recipientUserId: bigint, params: FindNotificationsDto) {
    ...
    const [data, total] = await Promise.all([
      this.db.client.cmsNotification.findMany({
        where,
        skip,
        take: pageSize,
        orderBy: { createdAt: 'desc' },
      }),
      this.db.client.cmsNotification.count({ where }),
    ]);
  ...
  async markAsRead(id: bigint, recipientUserId: bigint) {
    const notification = await this.db.client.cmsNotification.findFirst({
    ...
    return this.db.client.cmsNotification.update({
      where: { id },
  ...
  async markAllAsRead(recipientUserId: bigint) {
    const result = await this.db.client.cmsNotification.updateMany({
  ...
  async getUnreadCount(recipientUserId: bigint) {
    const count = await this.db.client.cmsNotification.count({
      where: { recipientUserId, isRead: false },
    });
  ...
  async createNotification(data: {
    ...
    return this.db.client.cmsNotification.create({ data });
  }
}

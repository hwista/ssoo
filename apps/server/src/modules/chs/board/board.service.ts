...
export class BoardService {
  ...
  async findAll(params: FindBoardsDto) {
    ...
    const [data, total] = await Promise.all([
      this.db.client.chBoard.findMany({
        where,
        skip,
        take: pageSize,
      ...
      this.db.client.chBoard.count({ where }),
    ]);
  ...
  async findOne(id: bigint) {
    const board = await this.db.client.chBoard.findUnique({
      where: { id },
      include: {
        categories: { where: { isActive: true }, orderBy: { sortOrder: 'asc' } },
        _count: { select: { posts: true } },
  ...
  async create(dto: CreateBoardDto) {
    const existing = await this.db.client.chBoard.findUnique({
    ...
    return this.db.client.chBoard.create({
      data: {
  ...
  async update(id: bigint, dto: UpdateBoardDto) {
    const existing = await this.db.client.chBoard.findUnique({ where: { id } });
    if (!existing || !existing.isActive) {
      throw new NotFoundException(`Board ${id} not found`);
    }

    return this.db.client.chBoard.update({
      where: { id },
      data: {

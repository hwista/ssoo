...
import { storageAdapterService, type StorageOrigin, type StorageStatus } from '../storage/storage-adapter.service.js';
...
class IngestQueueService {
  private getQueueRootPath(): string {
    const configured = configService.getConfig().ingest.queuePath || path.join(configService.getAppRoot(), 'data', 'ingest');
    return path.isAbsolute(configured) ? configured : path.join(configService.getAppRoot(), configured);
  }

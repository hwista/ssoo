...
| Shared Session Cookie | 7일 | PMS/SNS/DMS 간 세션 복원 bootstrap |
...
- refresh token 지속 저장은 제거하고, PMS/SNS/DMS 사이 세션 공유는 **HttpOnly shared session cookie** 로 처리합니다.
...
`checkAuth()` 는 localStorage access token을 권위 상태가 아니라 cache로 취급합니다. `/auth/me` 가 성공하면 현재 access token을 계속 사용하지만, 401 같은 권위 실패는 local auth state를 지우고 shared session restore도 서버 session row 검증을 통과할 때만 성공합니다. 일시적 네트워크 실패만 명시적으로 transient로 분류해 UI 안정성을 유지할 수 있습니다.

Protected layout bootstrap은 최초 hydration뿐 아니라 tab/window `focus` 와 `visibilitychange: visible` 에서 debounce된 `checkAuth()` 를 다시 호출합니다. 다섯 로컬 앱은 포트가 달라 cross-origin 이므로 `localStorage` 이벤트 전파가 아니라 서버 session row 상태로 수렴해야 합니다.
...
- **공용**: 로그인, shared session bootstrap(`/auth/session`), 토큰 갱신, 로그아웃, `/auth/me`, `ssoo-auth` 저장 형식, auth store/runtime, login UI shell/card/copy/footer
- **로그인 UI surface 정렬**: `SharedAuthLoginPage` 가 `AuthPageShell` 을 직접 소유하며, Admin/CRM/PMS/DMS/SNS 의 `(auth)/layout.tsx` 는 app-specific chrome/theme 를 덧씌우지 않는다. 5앱 `/login` route 는 app-specific `appName`/`appDescription` 을 넘기지 않고 동일한 SSOT 로그인 화면을 사용한다. 플랫폼 명칭 확정 전까지 login surface 는 `SSOT` 로고, `로그인` 제목, `© 2026 SSOT` 푸터만 유지하고 설명성 tagline/copy 를 노출하지 않는다. 비밀번호 찾기, 가입 요청, 사내 SSO, Microsoft 365 로그인 action 은 `NEXT_PUBLIC_AUTH_*` URL 또는 `SharedAuthLoginPage` props 로 설정될 때만 같은 공용 카드 안에 노출한다. 기본 env 기반 provider 는 사내 SSO와 Microsoft 365만 포함하고, generic OAuth/Google 같은 추가 provider는 제품/보안 결정 전에는 기본 surface에 노출하지 않는다. 회원가입은 기본적으로 open signup보다 `가입 요청` 링크를 우선하며, 셀프 회원가입 허용 여부는 별도 결정이 필요하다.
- **공용 browser identity**: `AuthIdentity` 는 이제 `userId`, `loginId`, `userName` 만 담고, role/org/admin 성격 정보는 browser auth payload 에서 제거
...
- **계정 UI surface 정렬**: Admin/CRM/PMS/DMS/SNS 사용자 메뉴의 global account/profile/security entry 는 `@ssoo/web-auth` `AuthUserMenu` account center action 으로 수렴하고, SNS account center(`NEXT_PUBLIC_SNS_APP_URL`)로 이동한다. DMS 문서 도메인 설정은 별도 `설정` action 으로 유지한다.
...
| AUTH-06 | Microsoft 365 / 사내 SSO 실제 backend 연동 방식 확정 | P2 | 🔲 대기 |
| AUTH-07 | 셀프 회원가입 허용 여부와 가입 요청 승인 플로우 확정 | P2 | 🔲 대기 |
| AUTH-08 | 비밀번호 찾기/재설정 채널(메일/관리자 승인/SSO-only) 확정 | P2 | 🔲 대기 |

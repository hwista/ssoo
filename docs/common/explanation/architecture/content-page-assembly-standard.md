...
content-area에 탭이 바인딩되는 경로는 앱 `ContentArea`에서 임의 `renderTab(): ReactNode`로 처리하지 않는다. 앱은 `SsooRegisteredMdiContentArea`에 `defineSsooMdiPageRegistry()`로 만든 route registry를 주입한다. `SsooMdiTabbedContentArea`는 `web-shell` 내부 구현이며 root public API로 앱에 노출하지 않는다.
...
- 문서 페이지, 설정 페이지, AI 대화, 기존 DMS AI 검색은 `contentPage`다.
- 기존 DMS AI 검색의 page body, toolbar, results panel, AI sidecar panel, search utility는 `@ssoo/web-shell`의 `SsooAiSearchPage` 계열 공용 모듈이 소유한다. DMS `/ai/search`는 DMS 검색 API, 권한, 문서 열기, AI 첨부 action, DMS 문서 card renderer만 주입하는 adapter다.
- 전역 검색은 `SsooGlobalSearchPage` adapter를 통해 같은 `SsooAiSearchPage` 공용 모듈을 소비하는 `contentPage`다. 앱은 검색 API adapter와 결과 열기 action만 소유한다. `/ssoo/search`는 DMS 문서 AI 검색 sidecar 콘텐츠를 임의로 재사용하지 않고, 전역 검색 전용 sidecar가 정의되기 전까지 공용 `sidecarMode="hidden"` 경로를 사용한다.
...
8. `check:golden-example` 또는 공통 검증 스크립트가 app-local recipe clone, page surface class 누수, `legacyException` route kind 재유입을 탐지하도록 확장한다.

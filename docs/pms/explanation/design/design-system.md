...
>
  ...
  <div className="h-[60px] bg-ssoo-primary flex items-center justify-between px-3">
    <div className="flex items-center gap-2">
      ...
      <span className="text-title-card text-white">SSOO</span>
    </div>
...
| 파일 | 역할 | 수정 가능 |
|------|------|----------|
| `globals.css` | CSS 변수 + 기존 alias 유지 | ✅ 전역 적용 |
| `tailwind.config.ts` | `fontFamily`, semantic `fontSize` 정의 | ✅ 정본 구현 |
| 컴포넌트 | 상속받음 | ❌ 개별 정의 금지 |
...
### Density-Driven Typography Model

- PMS typography는 `slot 역할 -> density -> 허용 token` 순서로 선택합니다.
...
- `Sidebar.itemLabel -> body-sm 또는 label-md`
- `Sidebar.meta -> detail`
- `Badge/Chip.text -> badge 또는 label`

### Guard Rules

- raw 조합(`text-sm font-medium`, `text-xs font-medium`, `text-sm font-semibold`, `text-lg font-semibold`, `text-xs font-semibold`, `text-lg font-medium`, `body-text font-medium`)은 금지합니다.
...
- `badge/chip/table cell` 내부도 역할/밀도 기준 정규화 대상입니다.

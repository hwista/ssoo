...
> 📍 원본 위치: `packages/database/scripts/`  
> 📍 아카이브 일자: 2026-01-23  
> 📍 아카이브 사유: SQL 기반 시드로 전환 완료
...
아래 스크립트는 **현재 활성** 상태로 `packages/database/scripts/`에 유지됩니다:
...
- 시드 데이터는 `packages/database/prisma/seeds/*.sql` 기준으로 관리

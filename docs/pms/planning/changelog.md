...
| 시간 | 커밋 | 영역 | 변경 내용 |
|------|------|------|----------|
| - | - | DB | **System Instance 마스터 추가**: `cm_system_instance_m`, `cm_system_instance_h`, 히스토리 트리거 및 메뉴/코드 시드 반영 |
| - | - | API | **System Instance CRUD 엔드포인트 추가**: 목록, 트리, 상세, 생성, 수정, 비활성화 구현 |
| - | - | UI | **관리자 시스템 인스턴스 페이지 추가**: 고객사/사이트/시스템종류/운영유형 필터, 계층 렌더링, 생성/수정/비활성화 지원 |
| - | - | 문서 | **도메인 용어 현행화**: `Plant/Site` 표현을 `Site` 기준으로 정리 |
| - | - | DB | **Site 마스터 추가**: `cm_site_m`, `cm_site_h`, `SITE_TYPE` 코드 및 히스토리 트리거/메뉴 시드 반영 |
| - | - | API | **Site CRUD 엔드포인트 추가**: 목록, 트리, 상세, 생성, 수정, 비활성화 구현 |
| - | - | UI | **관리자 사이트 페이지 추가**: 고객 필터, 계층 렌더링, 생성/수정/비활성화 지원 |
| - | - | DB | **프로젝트 Site 참조 정합화**: `plant_id`를 `site_id`로 변경 |
| - | - | DB | **System Catalog 마스터 추가**: `cm_system_catalog_m`, `cm_system_catalog_h`, 히스토리 트리거 및 시드/메뉴 데이터 반영 |
| - | - | API | **System Catalog CRUD 엔드포인트 추가**: 목록, 트리, 상세, 생성, 수정, 비활성화 구현 |
| - | - | UI | **관리자 시스템 카탈로그 페이지 추가**: 검색, 들여쓰기 렌더링, 생성/수정/비활성화 지원 |
| - | - | DB | **프로젝트 핸드오프 수신자 컬럼 추가**: `pr_project_m`, `pr_project_h`, 히스토리 트리거에 `handoff_user_id` 반영 |
| - | - | API | **핸드오프 H01~H03 엔드포인트 추가**: 생성, 수락, 완료 액션 구현 |
| - | - | UI | **프로젝트 상세 핸드오프 섹션 추가**: 수신자 선택, 수락/완료 액션, 오너 자동 승계 연결 |
| - | - | 문서 | **핸드오프 액션 스펙 현행화**: H01/H02/H03 구현 상태 반영 |

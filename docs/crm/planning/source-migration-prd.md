...
현재 SSOO 저장소에는 CRM 웹 앱, 서버 샘플 API, 공유 타입, Docker 3001 포트 골격이 들어와 있다. 다만 현재 화면과 API는 DB 원장 기반 구현이 아니라 fixture 조회 기준선이다. 따라서 지금 단계는 외부 데모를 추가로 복제하는 단계가 아니라, 먼저 샘플/미구현 상태를 실제 업무 데이터처럼 보이지 않게 낮추고 SSOO 업무 화면 품질에 맞는 목록·상세·저장 구조를 단계적으로 닫는 단계다.
...
| 현재 SSOO 저장소 | 확인 | CRM 웹 앱·서버 샘플 API·공유 타입·문서·Docker 3001 포트 골격이 존재한다. 다만 데이터는 fixture이며 DB 영속화, 저장/수정 API, 견적/계약/청구/정산은 아직 후속 slice이다. |
...
| SSOO 내 CRM 구현 | 20% | CRM 웹 앱, 서버 모듈, 공유 타입, compose 3001 포트 골격은 있으나 데이터는 fixture이고 DB 영속화/저장 API/권한/상세 상호작용은 없다. |
| CRM 문서 기준선 | 65% | 기능 범위와 단계 계획은 정리됐지만 상세 필드/DB 모델/검증 스크립트/실제 PMS 패턴 재사용 기준은 다음 단계 필요. |
| 런칭 가능 CRM | 5% | 재현 가능한 소스 골격은 있으나 실 데이터, 저장/수정, 권한, 계약 전환, 감사/잠금이 없어 런칭 가능 상태가 아니다. |
| 1단계 영업기회 이식 준비 | 30% | 원천 업무 구조와 fixture API/목록/검색/필터/정렬 화면은 있으나 등록/수정 저장, 선택 상세, 매출/원가 입력, DB 모델은 미착수다. |
...
3. 등록/수정 저장 API와 확정 후 잠금 검증을 추가한다.
4. 영업기회 상세 화면에서 매출/원가 라인 입력과 손익 계산을 실제 저장 흐름까지 닫는다.
5. 계약·견적·DMS·PMS 연결은 미구현 상태를 유지하고, 실제 연결은 별도 승인된 다음 단계로 분리한다.
6. PMS에는 읽기용 인계 후보/스냅샷만 전달한다는 경계를 화면 문구와 백로그에 계속 고정한다.
7. 빌드, Docker 반영, 디스크 상태를 각 slice마다 함께 검증한다.

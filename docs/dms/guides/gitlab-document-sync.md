...
DMS 웹 사이드바의 `publish 복구`는 runtime markdown Git working tree 전체 변경사항이 아니라 자동 publish 실패/차단 항목만 보여줍니다. 정상 운영에서는 사용자가 직접 커밋하지 않습니다.
...
- `publish 복구` 섹션은 `sync-blocked`/`push-failed` 상태가 있을 때만 표시되며, 버튼은 local commit만 하지 않고 서버의 자동 publish 재시도(commit + push) 흐름을 다시 큐잉합니다.
...
- DMS 웹 사이드바에 정상 대기 항목이나 수동 `커밋` 버튼이 노출되지 않음
- 실패 시에만 `publish 복구` 섹션이 표시되고, 원인 조치 후 `수동 publish`로 재시도 가능

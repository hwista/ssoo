...
| | `/api/storage/open` | 참조 파일 열기 URL/링크 반환 (`local` provider는 `documentPath` 필요) |
...
- `POST /api/storage/open`
  - body: `{ storageUri?, provider?, path?, documentPath? }`
- `GET /api/storage/open`
  - query: `storageUri|provider|path`
  - `provider=local` 인 경우 `documentPath` query가 추가로 필요합니다.

정책:

- `sharepoint`/`nas` 는 feature gate(`canManageStorage`) 기준으로만 검증합니다.
- `local` 은 **읽을 수 있는 문서에 실제로 연결된 sourceFile** 인 경우에만 열 수 있습니다.
- 즉, `documentPath` 가 필요하고, 해당 문서 sidecar `sourceFiles` 안에 요청한 `path` 또는 `storageUri` 와 일치하는 항목이 있어야 합니다.
...
위키 디렉토리 내 이미지 파일을 raw 바이너리로 반환합니다. 브라우저 요청에 `Authorization` 헤더가 없더라도, same-origin 프록시가 shared session cookie로 access token을 복원한 뒤 server `canReadDocuments` 권한을 검사합니다.
...
첨부파일을 서빙하거나 다운로드합니다. same-origin 프록시는 shared session cookie로 access token을 복원할 수 있으며, server는 `canReadDocuments` 권한을 다시 검사합니다.

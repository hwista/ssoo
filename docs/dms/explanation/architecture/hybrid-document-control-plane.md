...
이 문서는 DMS 문서 런타임을 **file/Git content plane + DB control-plane + sidecar projection** 으로 분리하는 이유와 책임 경계를 정의합니다.
...
| sidecar | file 옆 투영된 문서 메타데이터 | generated projection |

핵심 원칙:

1. sidecar 와 DB를 독립 writable truth 두 개로 두지 않는다.
2. file/Git 은 내용(content)의 정본이고, DB 는 제어(control)의 정본이다.
3. sidecar 는 호환성/운영 편의를 위한 projection 이다.
...
- sidecar-like metadata 는 구조가 자주 바뀔 수 있어 `jsonb` 가 유리하다
...
- sidecar / API / DB payload 모두 `sourceFiles` 기준으로 수렴한다

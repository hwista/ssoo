...
DMS는 이제 pnpm workspace 앱으로 통합되었고, PMS와는 다음 아키텍처 차이점이 남아 있습니다:

| 항목 | PMS | DMS |
|------|-----|-----|
| 인증 | JWT 기반 | JWT + shared auth storage(`ssoo-auth`) 기반, DMS shell은 로그인 필수 |
| API 토폴로지 | NestJS 백엔드 호출 | Next same-origin proxy + `apps/server` DMS 모듈 |

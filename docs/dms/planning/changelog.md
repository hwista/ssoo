...
2026-09-23 별도 설정 후속 승인 적용: 사용자 “승인”으로 권한 관리·계약 설정 2개·수집 큐·관리자 템플릿의 배치와 템플릿 조회 실패 안내/재조회를 적용했다. 기존 저장·권한·확정·서버 계약을 유지한다. 새 빌드·보호 검사와 기본 40상태·입력 배치 25상태·입력 및 복구 검사 26개를 확인했다. 중간 스크롤의 대화 버튼 가림은 별도 잔여이며 [핸드오프](2026-09-23-settings-detail-layout-handoff.md)를 따른다.

2026-09-22 검색 결과 표현 완료: 사용자 “적용하자” 승인으로 공통 검색 카드에 업무 명칭·선별 정보·이동 서비스 표시를 적용했다. 문서관리 전용 문서 카드·권한·검색 기록·도우미는 유지한다. 문서관리 브라우저 29개 검사와 보호 검사·새 빌드를 확인했다. 동적 요약을 분리한 전용 카드 2건의 고정 텍스트 보존 및 실제 문서 열기를 검증했다. [핸드오프](../../common/explanation/architecture/2026-09-22-search-results-handoff.md).

2026-09-22 로그인 옵션 완료: 사용자 “진행”으로 아이디 저장·비밀번호 표시를 기존 공용 로그인에 연결했다. 비밀번호 찾기 위치를 공용 옵션 배치로 맞추며 기존 인증·문서 권한·저장·배포 계약을 유지한다. 새 빌드·보호 검사와 문서관리 28개 실제 동작 검증, 네 화면 폭을 확인했다. [핸드오프](../../common/explanation/architecture/2026-09-22-login-options-handoff.md).

2026-09-22 설정 입력 행 배치 완료: 사용자 “진행”으로 개인 설정 4개·시스템 설정 6개의 항목명/입력란 겹침을 보완했다. 입력 행의 실제 가용 폭이 좁을 때 세로 배치하며 기존 공용 입력 원자·값·검증·저장 계약을 보존한다. 새 빌드·보호 검사·브라우저 102상태와 입력 유지·초기화·오류 안내를 검증했다. [핸드오프](2026-09-22-settings-field-layout-handoff.md). 아래 상단 배치 기록에서 남겼던 설정 내부 잔여의 후속 처리이며 설정 전체 기능 완료 판정은 아니다.

2026-09-22 승인 상단 배치 완료: 사용자 “좋아 진행”으로 768~1023px 검색 상단을 두 줄·104px로 배치했다. 알림창은 실제 상단 아래를 따라가며 검색 후 입력 비우기·모바일·검색 없는 설정 상단·문서/저장 계약은 보존했다. [공용 핸드오프](../../common/explanation/architecture/2026-09-22-header-search-layout-handoff.md). 다섯 새 빌드·실제 검색 및 설정 상단 세 크기를 확인했다. 좁은 설정 내부의 항목명 겹침은 기존 미정리 잔여이며 이번 완료에 포함하지 않는다.

2026-09-18 승인-15 완료: [핸드오프](../../common/explanation/architecture/2026-09-18-document-diagram-handoff.md). 네 종류 도식의 읽기·편집/템플릿 미리보기·인쇄, 원문 전환·검색·일시 실패 재시도와 좁은 화면 도구 배치 보완. 원문/계약 보존·기존 업무 11/11·다섯 웹 빌드 통과. 21완료·1잔여·운영 증거 0/5. 아래는 이전 단계 기록이다.

2026-09-17 승인 준비: [다이어그램 적용안](2026-09-17-diagram-approval-proposal.md). 두 도식의 읽기·미리보기·인쇄 글 표시와 휴대폰 도구 겹침 재현, 원문 보존·실패 복구·좁은 화면 배치 범위 제시. 제품 변경 없음, 기존 승인-15 대기 유지.

2026-09-17 알림 후속: [연결 오류](../../common/explanation/architecture/2026-09-17-notification-stream-handoff.md)를 실제 브라우저에서 재현하고 공통 프록시의 이전 본문 길이·압축·전송 정보를 제거했다. 기존 화면·요청/응답 데이터·재시도·권한·다운로드는 보존한다.

2026-09-17 후속: 고객관리의 [계약 초안 내부 승인](../../common/explanation/architecture/2026-09-17-contract-internal-approval-handoff.md) 구현·검증 완료. 기존 문서 열람 권한과 문서 등록 절차를 재사용하며 문서관리 화면·기존 연동 계약은 변경하지 않았다. 기존 문서관리 회귀 4/4와 필수 가드 통과.

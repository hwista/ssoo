-- =========================================================
-- History Trigger: sns.sns_comment_m -> sns.sns_comment_h
-- Schema: sns
-- =========================================================

CREATE OR REPLACE FUNCTION fn_sns_comment_h_trigger()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  INTO v_history_seq
  FROM sns.sns_comment_h
  WHERE comment_id = v_record.comment_id;

  INSERT INTO sns.sns_comment_h (
...
DROP TRIGGER IF EXISTS trg_sns_comment_h ON sns.sns_comment_m;
CREATE TRIGGER trg_sns_comment_h
AFTER INSERT OR UPDATE OR DELETE ON sns.sns_comment_m
FOR EACH ROW EXECUTE FUNCTION fn_sns_comment_h_trigger();
COMMENT ON FUNCTION fn_sns_comment_h_trigger() IS '댓글 마스터 히스토리 트리거 함수';

-- =========================================================
-- History Trigger: cms.cms_board_m -> cms.cms_board_h
-- Schema: cms
-- =========================================================

CREATE OR REPLACE FUNCTION fn_cms_board_h_trigger()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  INTO v_history_seq
  FROM cms.cms_board_h
  WHERE board_id = v_record.board_id;

  INSERT INTO cms.cms_board_h (
...
DROP TRIGGER IF EXISTS trg_cms_board_h ON cms.cms_board_m;
CREATE TRIGGER trg_cms_board_h
AFTER INSERT OR UPDATE OR DELETE ON cms.cms_board_m
FOR EACH ROW EXECUTE FUNCTION fn_cms_board_h_trigger();
COMMENT ON FUNCTION fn_cms_board_h_trigger() IS '게시판 마스터 히스토리 트리거 함수';

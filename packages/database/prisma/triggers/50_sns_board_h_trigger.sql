-- =========================================================
-- History Trigger: sns.sns_board_m -> sns.sns_board_h
-- Schema: sns
-- =========================================================

CREATE OR REPLACE FUNCTION fn_sns_board_h_trigger()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  INTO v_history_seq
  FROM sns.sns_board_h
  WHERE board_id = v_record.board_id;

  INSERT INTO sns.sns_board_h (
...
DROP TRIGGER IF EXISTS trg_sns_board_h ON sns.sns_board_m;
CREATE TRIGGER trg_sns_board_h
AFTER INSERT OR UPDATE OR DELETE ON sns.sns_board_m
FOR EACH ROW EXECUTE FUNCTION fn_sns_board_h_trigger();
COMMENT ON FUNCTION fn_sns_board_h_trigger() IS '게시판 마스터 히스토리 트리거 함수';

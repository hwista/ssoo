-- =========================================================
-- History Trigger: pms.pr_milestone_m -> pms.pr_milestone_h
-- Schema: pms
-- =========================================================

CREATE OR REPLACE FUNCTION fn_pr_milestone_h_trigger()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  INTO v_history_seq
  FROM pms.pr_milestone_h
  WHERE milestone_id = v_record.milestone_id;
...
DROP TRIGGER IF EXISTS trg_pr_milestone_h ON pms.pr_milestone_m;

CREATE TRIGGER trg_pr_milestone_h
AFTER INSERT OR UPDATE OR DELETE ON pms.pr_milestone_m
FOR EACH ROW EXECUTE FUNCTION fn_pr_milestone_h_trigger();

COMMENT ON FUNCTION fn_pr_milestone_h_trigger() IS '마일스톤 히스토리 트리거 함수';

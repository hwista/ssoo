/**
 * Ask Handler - AI 질문 응답을 담당하는 핸들러 (벡터 검색 기반)
 * Route: /api/ask
 */
...
export async function askQuestion(
  ...
  if (!question || typeof question !== 'string') {
    return { success: false, error: '질문이 필요합니다', status: 400 };
  }

  if (!process.env.GEMINI_API_KEY) {
    return { success: false, error: 'GEMINI_API_KEY가 설정되지 않았습니다', status: 500 };
  }
...
## 답변`;
    ...
    // 5. 소스 정보 정리
    const sources: SourceInfo[] = searchResults.map(r => ({

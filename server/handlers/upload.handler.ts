/**
 * Upload Handler - 파일 업로드 관련 작업을 담당하는 핸들러
 * Route: /api/upload
 */
 ...
 * 파일 업로드 처리
 */
export async function handleUpload(file: File): Promise<HandlerResult<{
  message: string;
  data: UploadResult;
}>> {
  try {
    ...
    if (file.size > maxSize) {
      return { success: false, error: '파일 크기는 10MB 이하여야 합니다', status: 400 };
    }
 ...
 * 업로드된 파일 목록 조회
 */
export async function getUploadList(): Promise<HandlerResult<UploadListItem[]>> {
  try {
    await ensureDirectories();
    ...
    return { success: false, error: '파일 목록 조회 중 오류가 발생했습니다', status: 500 };
  }
}
